...
def ensure_https_and_no_trailing_slash(url):
    has_protocol = re.match(r"^[a-z]+://", url, re.I)
    modified_url = url if has_protocol else 'https://' + url

    return modified_url[:-1] if modified_url.endswith('/') else modified_url
